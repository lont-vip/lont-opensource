			...
			if($trade['num']>$trade['deal']){
				if ($trade['type'] == 1) {
					...
					try{
						...
						if(check_arr($rs)) {
							$mo->table('tw_finance')->add(array('userid' => $userid,'type'=>1, 'coin' => $xnb, 'coinname' => strtoupper($xnb),'remark'=>'币安交易','remark2'=>'卖'.strtoupper($rmb),'num'=>$usercoin[$xnb],'fee'=>$money_turnover,'mum'=>($usercoin[$xnb]+$money_turnover), 'addtime' => time()));
							if($money_fee>0){
								$mo->table('tw_finance')->add(array('userid' => $userid,'type'=>1, 'coin' => $xnb, 'coinname' => strtoupper($xnb),'remark'=>'币安交易[手续费]','remark2'=>'卖'.strtoupper($rmb),'num'=>($usercoin[$xnb]+$num),'fee'=>$money_fee,'mum'=>($usercoin[$xnb]+$money_turnover+$money_fee), 'addtime' => time()));
							}
							//发送到币安
							...
							$mo->rollback();
							$this->error(L('撤销失败'));
							throw new \Think\Exception(L('交易失2败！'));
						}
					...
					try{
						...
						$rs[] = $mo->table('tw_trade')->where('id='.$trade['id'])->save(array('status' => 2, 'time_cx' => time()));
						if(check_arr($rs)) {
							$mo->table('tw_finance')->add(array('userid' => $userid,'type'=>1, 'coin' => $rmb, 'coinname' => strtoupper($rmb),'remark'=>'币安交易','remark2'=>'卖'.strtoupper($rmb),'num'=>$usercoin[$rmb],'fee'=>$chexiao_num,'mum'=>($usercoin[$rmb]+$chexiao_num), 'addtime' => time()));
							if($num_fee>0){
								$mo->table('tw_finance')->add(array('userid' => $userid,'type'=>1, 'coin' => $rmb, 'coinname' => strtoupper($rmb),'remark'=>'币安交易[手续费]','remark2'=>'卖'.strtoupper($rmb),'num'=>($usercoin[$rmb]+$num),'fee'=>$num_fee,'mum'=>($usercoin[$rmb]+$chexiao_num+$num_fee), 'addtime' => time()));
							}
							//发送到币安
							...
							if($orders['symbol']==$trade['market2']){
								...
								$mo->rollback();
								$this->error(L('撤销失败'));
								throw new \Think\Exception(L('撤销失败'));
							}
						} else {
							$mo->rollback();
							$this->error(L('撤销失败'));

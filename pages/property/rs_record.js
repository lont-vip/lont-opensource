loader.define(["main"],function(main,require,exports,module) {
    ...
    newspage.nav = function (argument) {
       ...
       slideHeight = $(window).height()  - $("#tabDynamicNav4").height()- $("#uiNewsTabNav4").height();
       // 新闻导航选项卡 js 初始化:
       uiNewsTab = bui.tab({
           id:"#uiNewsTab4",
           menu:"#uiNewsTabNav4",
       ...
       // 让顶部导航滚动到可视位置
        uiNewsTab.on("to",function (index) {
            ...
            var left = $("#uiNewsTabNav4 li")[index].offsetLeft - itemwidth*2;
            document.getElementById("uiSlideNavbar4").scrollLeft = left;

            var index = index,
                num = index +1 ;
            // 加载对应的脚本文件
              loader.require(["pages/property/rs_record"+num]);
        }).to(0)

loader.define(["main"],function(main,require,exports,module) {
    ...
    newspage.nav = function (argument) {
        ...
        var newspageMainHeight = bui.init({
            id:"#newsPage2",
            footer:"#tabDynamicNav2"
        });

       slideHeight = $(window).height()  - $(".adve-height").height();
       // 新闻导航选项卡 js 初始化:
       uiNewsTab = bui.tab({
           id:"#uiNewsTab2",
           menu:"#uiNewsTabNav2",
       ...
       // 让顶部导航滚动到可视位置
        uiNewsTab.on("to",function (index) {
            // 有滚动条时在居中显示
            var itemwidth = $("#uiNewsTabNav2 li").eq(index).prev().width();
            var left = $("#uiNewsTabNav2 li")[index].offsetLeft - itemwidth*2;
            document.getElementById("uiSlideNavbar2").scrollLeft = left;

            var index = index,
                num = index +1 ;
            // 加载对应的脚本文件
              loader.require(["pages/transaction/advertising"+num]);
        }).to(0)
